...
{
    public class WapAuthorizationAttribute : ActionFilterAttribute, IAuthorizationFilter
    {
        private bool IsValid { get; set; }
        public WapAuthorizationAttribute()
            : this(false)
        {
        }

        public WapAuthorizationAttribute(bool isValid)
        {
            IsValid = isValid;
        }
        ...
        public void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            {
                //是否登录（验证）
                if (!WapLoginUserManager.IsLogin)
                {
